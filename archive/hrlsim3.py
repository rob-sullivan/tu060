...
"""
#Gym has been moved to Gymnasium 
...
# https://github.com/WhatIThinkAbout/BabyRobotGym/blob/main/notebooks/Part%201%20-%20Creating%20the%20framework.ipynb
...
class HomeoRLEnv(gym.Env):
    ...
    in March 2013 using Python 2.6 to mimic experimental cocaine data.
    """
    def __init__(self, **kwargs): #**kwargs allows us to pass keyworded variable length of arguments to a function, e.g dictonaries.
        super().__init__()
        "actions = ['Sleep', 'Binge on Internet', 'Work', 'Exercise', 'Socialise', 'Drink Alcohol', 'Smoke']"
        "observations = [exState, inState, setpointS, num_of_4_secs]"


    #custom gym environment functions
    def step(self, action): #action
        #get our current observations
        exState = self.state[0]
        inState = self.state[1]
        setpointS = self.state[2]
        #trialCount = self.state[3] #this is time or how how many trials occured, e.g 0 + 900 = 900 trials or 1 hour

        #take an action then move 4 seconds into the future
        nextState = self.getRealizedTransition(exState,action)
        
        #get non homeostatic reward for that action
        ...
        #get non homeostatic reward for that action
        out = self.getOutcome(exState,action,nextState) #if state = 0 and action is active-lever and next state 1 then give 50 hit of self.cocaine
        HomeoRew = self.driveReductionReward(inState,setpointS,out) # used in value estimation function
        self.updateOutcomeFunction(exState,action,nextState,out)
        
        #estimate next assumed state
        self.updateTransitionFunction(exState,action,nextState)            
        
        #update internal sensors or observations of the environment
    ...
    def reset(self, seed=None, options=None):
        super().reset(seed=seed) #reset gym

        """Resets the environment for the RL agent"""
        #set number of states, actions, initial addiction values, leverpress values, etc.
        self.setupEnvironment()
        #setup Homeostatic System, Allostatic (Stress) System, Drive Function and Goal-directed system
        self.setupAnimal()

        #used for gym
        state = self.state #initially np array size 4 all zeros
        info = {}
        return state,info
    ...
    def getNonHomeostaticReward(self, state,action,nextState):
        ...
        #the weighted average of these two values is then used as the overall value assigned to a state-action pair
    ...
    def setupEnvironment(self, s=5, a=3):
        ...
        self.statesNum = s # sequence of steps that each take 4 second to complete. 5 states x 4secs = 20secs
        self.actionsNum = a # number of action, e.g action 0 = Null (do nothing), action 1 = Pull no-cocaine lever (Inactive Lever Press), action 2 = Pull cocaine lever (Active Lever Press)
    ...
    def initializeAnimal(self):    
        ...
        self.state = np.zeros( [4] , int) # the observation space or sensors of the agent, a vector of the external state, internal state, setpoint, and trial
        ...
        #initialise the transition table (grid world), outcome table and the non-homeostatic-reward table
        for i in range(0, self.statesNum):
            for j in range(0, self.actionsNum):
                for k in range(0, self.statesNum):
                    self.estimatedTransition[i][j][k] = (1.0)/( self.statesNum*1.0) #where agent thinks it will be
                    self.estimatedOutcome[i][j][k] = 0.0 #what cocaine hit agent thinks it will get
                    self.estimatedNonHomeostaticReward[i][j][k] = 0.0 #what update to its internal homeostatic setpoint it thinks it will get
    ...
    def setupKeramatiParameters(self):
        ...
        self.pretrainingHours = 1 #hours spent pretraining
        self.sessionsNum = 1 # Number of sessions of cocain seeking, followed by rest in home-cage
        self.seekingHoursShA = 3 #hours spent seeking cocaine for short administration rats
        self.seekingHoursLgA = 3 #hours spent seeking cocaine for long administration rats
        self.extinctionHours = 0 #hours spent without cocaine

        self.trialsPerHour = 900 # 60*60/4 = 3600/4 = 900 trials per hour (each trial is supposed to last 4 seconds where rat could choose cocaine or not)
    ...
    #Keramati simulated experiments
    def pretraining(self, ratType):
        ...
        exState = self.state[0]#current state
        inState = self.state[1]#internal variable
        setpointS = self.state[2]#hs setpoint
        trialCount = self.state[3] # time elapsed in time instances of 4 secs e.g start at 0
        ...
        trialsNum = self.pretrainingTrialsNum # 900 trials * 1 hour (1 trial = 4secs)
        
        for trial in range(0,trialsNum):#0 to 900
            """
            ...
            nonHomeoRew = self.getNonHomeostaticReward(exState,action,nextState)
            HomeoRew = self.driveReductionReward(inState,setpointS,out) #homeoRew used in valueEstimation() function.
        ...
        self.state[1] = inState
        self.state[2] = setpointS
        self.state[3] = trialCount+trialsNum # 0+900 = 900 trials or 1 hour of time
    ...
    def noncontingentInfusion(self, sessionNum,ratType):
        """Non-contingent cocaine infusion. Directly give cocaine to animal (usually by injection) 
        without animal needing to press a lever, etc."""
        exState = self.state[0]
        inState = self.state[1]
        setpointS = self.state[2]
        trialCount = self.state[3]
    ...
    def runKeramatiExperiment(self):
        ...
        #pretrain agent
        self.pretraining('ShA') #pretrains for an hour
        ...
        for session in range(0, self.sessionsNum):
            self.cocaineSeeking(session, 'ShA') #seeks cocaine for about 3 hours
