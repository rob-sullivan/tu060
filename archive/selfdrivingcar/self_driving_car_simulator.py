...
class Game(Widget):
    ...
    def update(self, dt): # the big update function that updates everything that needs to be updated at each discrete time t when reaching a new state (getting new signals from the sensors)
        ...
        global reward_received # specifying the global variables (the last reward)
        ...
        action = brain.update(reward_received, last_signal) # playing the action from our ai (the object brain of the dqn class)
        scores.append(brain.score()) # appending the score (mean of the last 100 rewards to the reward window)
        rotation = agent_actions[action] # converting the action played (0, 1 or 2) into the rotation angle (0°, 20° or -20°)
...
# %%
CarApp().run()#closing application will register as a crash in juypter notebook, just ignore.
